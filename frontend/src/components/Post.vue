<template>
  <div class="base_conference">
    ...
    <el-container class="wrap_container">
      ...
      <!-- 回复帖(回复主题帖) -->
      <el-container class="content_container" v-for="(reply,index) in post.replyList" v-bind:key="index" v-if="reply.replyToFloorNumber === -1">
      <el-aside>
        <span><i class="el-icon-user-solid"></i>{{reply.ownerFullName}}</span>
      </el-aside>
      <el-main>
        <div class="words">{{reply.words}}</div>
        <div class="show_reply_container"><el-button type="text" @click="showReply=!showReply" class="show-reply-button">显示回复</el-button></div>
        <!-- 对回复帖的回复 -->
        <div class="comment_container" v-show="showReply">
          <div class="comment" v-for="(comment,index) in post.replyList" v-bind:key="index" v-if="comment.replyToFloorNumber === reply.floorNumber">
            <div class="words"><i class="el-icon-user-solid"></i><span>{{comment.ownerFullName}}:</span> {{comment.words}}</div>
          ...
          <el-button @click="replyTo(null)" class="show-reply-input-button">我来说一句</el-button>
            ...
            <el-button
            type="primary" @click="replyPost(reply.floorNumber)" class="submit-reply-button">发表</el-button>
        </div>
      </el-main>
      </el-container>
  ...
  export default{
    ...
    created(){
      ...
      if(this.$route.params.articleID){
        const _this = this;
        this.$axios.post('/browsePostOnArticle',{
